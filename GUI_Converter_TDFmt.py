"""
20200803 增加：txtedit文本编辑器增加内容后自动跳转到文末
"""
...
class SplitterExample(QWidget):
    ...
    def initUI_topLeft(self):
        ...
        self.lineEdit2.setValidator(dbl_validator)
        # 参数：参考长度
        self.lineEdit3 = QLineEdit()
        self.lineEdit3.setValidator(dbl_validator)
        # 参数：舵偏角
        lable4 = QLabel()
        ...
        # 参数：原始数据文件名（.plt）
        lable_plt = QLabel("原始数据文件名")
        self.lineEdit_plt = QLineEdit()
        self.pushbutton_plt = QPushButton("选择")
        # 参数：原始数据文件夹
        ...
        formLayout.addRow("车次文件", hboxSerial)
        hbox1 = QHBoxLayout()
        hbox1.addWidget(self.radioButton_2)
        hbox1.addWidget(self.radioButton_3)
        formLayout.addRow("半模/全模", hbox1)
        formLayout.addRow("海拔高度（m）", self.lineEdit2)
        formLayout.addRow("参考长度（m）", self.lineEdit3)
        formLayout.addRow("舵偏角（°）\ndf dfa da de dr", self.lineEdit4)
        hbox5 = QHBoxLayout()
        hbox5.addWidget(self.lineEdit5)
        hbox5.addWidget(self.pushbutton5)
        formLayout.addRow(lable5, hbox5)
        hboxplt = QHBoxLayout()
        hboxplt.addWidget(self.lineEdit_plt)
        hboxplt.addWidget(self.pushbutton_plt)
        formLayout.addRow(lable_plt, hboxplt)
        hbox6 = QHBoxLayout()
    ...
    def initUI_connect(self):
        ...
        self.pushbutton_plt.clicked.connect(self.selectpltfile)
    ...
    def convert(self):
        ...
        # 20200803 增加：txtlog文本编辑器增加内容后自动跳转到文末
        cursor = self.txtlog.textCursor()
        cursor.movePosition(QTextCursor.End)  # 还可以有别的位置
        self.txtlog.setTextCursor(cursor)  # 把鼠标设置为有效setTextCursor，跳转
    ...
    def selectpltfile(self):
        if os.path.isdir(self.lineEdit5.text()):
            folder = self.lineEdit5.text()
        else:
            folder = r"C:\\"
        filename = QFileDialog.getOpenFileName(self, 'select pltfile', folder, "plt file(*.plt)")
        if filename[0]:
            self.pltname = os.path.split(filename[0])[1]
            self.lineEdit_plt.setText(self.pltname)
            ...
            while i < len(lns):
                ...
                if (lnlst[0] == '#'):
                    ...
                    if (len(lnlst) > 1 and lnlst[1] == 'halfmodel'):
                        self.halfmodel = True if delete_tail(lns[i]).lower() == 'true' else False
                    ...
                    elif (len(lnlst) > 1 and lnlst[1] == 'pltname'):
                        self.pltname = delete_tail(lns[i])
    ...
    def showcontrol(self):
        ...
        self.radioButton_3.setChecked(not self.halfmodel)
    ...
    def updata(self):
        ...
        self.pltname = self.lineEdit_plt.text()
    ...
    def autoupdata_outAssem(self):
        ...
        if "sumAssem" in lns:
            ...
            txtOutAssem = "%20s %20s\n" % ("QJ", "quanji")
        for line in lns:
            if line != 'quanji' and line != 'sumAssem':
                txtOutAssem += "%20s %20s\n" % (line, line)
    ...
    def saveFile(self):
        print(self.serial)
        if not os.path.isdir(os.path.split(self.serial)[0]):
            os.makedirs(os.path.split(self.serial)[0])
            self.txtlog.append("新创建文件夹及文件" % self.serial)
